...
Exercise 01 - Element displays / visible
    ...
    ${start_time}=  Get Time
    Log  Time start: ${start_time} seconds
    Wait Until Element Is Visible  ${Facebook_Textbox_Email}  10
    ${end_time}=  Get Time
    Log  Time end: ${end_time} seconds
...
Exercise 04: Element Presence -> element is in DOM, even if the element displays or not
    ...
    ${div_exist}=  Run Keyword And Return Status  Element Should Be Visible  //div[text()='Re-enter email address']
    Run Keyword If  "${div_exist}"=="${True}"  Wait Until Element Is Visible  //div[text()='Re-enter email address']  10
